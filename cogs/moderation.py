...
class ModCommands(commands.Cog):
    ...
    async def memeber(self, ctx, user: discord.Member):
        ...
        if result and result[12] != 0:
            ...
            await self.remove_member_role(
                ctx, user, ctx.guild.get_role(settings.config["statusRoles"]["member"])
            )
    ...
    async def mute(self, ctx, user: discord.Member, *, reason=None):
        ...
        if muted:
            await self.client.wait_until_ready()
            double_role = ctx.guild.get_role(
            ...
            if reason is None:
                ...
                await user.add_roles(
                    ctx.guild.get_role(settings.config["statusRoles"]["muted"])
                )
                await utils.doembed(
                    ctx,
                    "Mute",
    ...
    async def unmute(
        ...
        if user is None:
            ...
            self_muted = False
            double = False
            self_mute_role = ctx.guild.get_role(
                settings.config["statusRoles"]["self-mute"]
            )
    ...
    async def nlynch(self, ctx, member: discord.Member = None):
        ...
        if result:
            ...
            if datetime.utcnow() > (
                datetime.fromtimestamp(result[4]) + timedelta(hours=8)
            ):
                current_lynches = 1
                make_historical_query = text(
                    f"update mod_event set historical = 1 "
