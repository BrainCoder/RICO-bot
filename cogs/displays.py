import utils
from resources.rules import rules_dict
...
class SetReaction(commands.Cog):
    ...
    async def BuildRulesEmbed(self, ctx):
        embed = discord.Embed(
            title="Rules",
            url="https://tenor.com/view/idiot-lafuddyduddy-rules-follow-the-rules-penguin-gif-16685859",
            color=0x00DCFF,
        )
        embed.set_author(
            name=ctx.guild.name, url=self.invite, icon_url=ctx.guild.icon_url
        )

        for rule in rules_dict:
            embed.add_field(
                name=f"Rule {rule} - {rules_dict[rule][0]}",
                value=rules_dict[rule][1],
                inline=False,
            )

        embed.set_footer(
            text="NoPorn Companion was made by the NoPorn development team, please DM the bot for more information"
        )
    ...
    async def BuildWelcomeEmbed(self, ctx):
        embed = discord.Embed(
            title="Welcome",
            url="https://www.youtube.com/watch?v=dTS2Yya9Jj4&list=LLhv0ss-9EZsbjxEU67Rawsg&index=38",
            color=0x00DCFF,
        )
        embed.set_author(
            name=ctx.guild.name, url=self.invite, icon_url=ctx.guild.icon_url
        ...
        embed.add_field(
            ...
            been clean, learn more about this in <#{settings.config['channels']['streak-guide']}> where you can also assign yourself the mode that you're attempting, and to see the commands that you can apply there.",
            inline=False,
        )
    ...
    async def BuildChannelsEmbed(self, ctx):
        ...
        embed = discord.Embed(
            title="Channels",
            url="https://www.youtube.com/watch?v=0itU7sA765U",
            color=0x00FAA8,
        )
        embed.set_author(
            name=ctx.guild.name, url=self.invite, icon_url=ctx.guild.icon_url
    ...
    async def BuildMiscEmbed(self, ctx):
        embed = discord.Embed(
            title="Other Info",
            url="https://tenor.com/view/give-you-alittle-bit-more-info-abit-more-info-little-bit-info-information-info-gif-15208503",
            color=0x00FAA8,
        )
        embed.set_author(
            name=ctx.guild.name, url=self.invite, icon_url=ctx.guild.icon_url
        ...
        embed.add_field(
            name="Complaints",
            value=f"Send a direct message to <@{settings.config['botId']}> with your complaints if you have any. In order to do this, you need to have your DM's open",
        )
        embed.set_footer(
            text="NoPorn Companion was made by the NoPorn development team, please DM the bot for more information"
        )
