...
async def is_staff(user):
    ...
    staff_roles.append(
        await in_roles(user, settings.config["staffRoles"]["facilitator"])
    )
    staff_roles.append(
        await in_roles(user, settings.config["staffRoles"]["head-moderator"])
    )
    staff_roles.append(await in_roles(user, settings.config["staffRoles"]["moderator"]))
    staff_roles.append(
        await in_roles(user, settings.config["staffRoles"]["semi-moderator"])
    )
...
async def in_staff_channel(channel):
    channels = []
    channels.append(
        await in_channel(channel, settings.config["channels"]["development"])
    )
    channels.append(await in_channel(channel, settings.config["channels"]["testing"]))
    channels.append(
        await in_channel(channel, settings.config["channels"]["administration"])
    )
    channels.append(
        await in_channel(channel, settings.config["channels"]["moderators"])
    )
    channels.append(await in_channel(channel, settings.config["channels"]["semi-mods"]))
    channels.append(
        await in_channel(channel, settings.config["channels"]["staff-lounge"])
    )
    channels.append(
        await in_channel(channel, settings.config["channels"]["complaints"])
    )
